...
class SpeedVisualizer():
    ...
    def speed_calculator(self, msg: Odometry) -> tuple[float, float, float]:
        ...
        if dt > 0:
            ...
            # 임시값!! 오차가 엄청 큼
            # 회전 시 누적 오차가 너무 커요 ㅠㅠㅠㅠㅠㅠ
            self.dist = self.dist / 2
            
            velocity = self.dist / dt
        ...
        return self.filtered_speed, self.dist, dt
...
class DeadReckoningNode(Node): #ros 노드인 메인 클래스
    def __init__(self):
        ...
        self.odom_pub = self.create_publisher(
            Odometry, PUB_NAMESPACE+"odom", qos_profile_system_default
        )
    ...
    def odom_publisher(self):
        ...
        t.header.frame_id = TF_NAMESPACE+'odom'
        t.child_frame_id = TF_NAMESPACE+'base_link'
        ...
        odom.header.frame_id = TF_NAMESPACE+'odom'
        odom.child_frame_id = TF_NAMESPACE+'base_link'
    ...
    def motor_callback(self, data):
        ...
        #디버깅용
        # if self.speed != 0:
        #     self.get_logger().info(f"speed(odom: {self.speed:.2f}, motor: {data.speed}, diff: {data.speed/self.speed:.6f}")
